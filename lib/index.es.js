...
function pick$$1(obj$$1, arr) {
  var res$$1 = {};
  for (var i = 0, len = arr.length; i < len; i++) {
    var prop = arr[i];
    if (prop in obj$$1) {
      res$$1[prop] = obj$$1[prop];
    }
  }
  return res$$1;
}
...
function tryFilter$1(filter, doc, req) {
  try {
    ...
    return createError$1(BAD_REQUEST$1, msg);
  }
}
...
function invalidIdError$1(id) {
  var err;
  if (!id) {
    err = createError$1(MISSING_ID$1);
  } else if (typeof id !== 'string') {
    err = createError$1(INVALID_ID$1);
  } else if (/^_/.test(id) && !(/^_(design|local)/).test(id)) {
    err = createError$1(RESERVED_ID$1);
  }
  if (err) {
    throw err;
  }
...
function rev$1(doc, deterministic_revs) {
  ...
  var mutateableDoc = assign$2$$1({}, doc);
  delete mutateableDoc._rev_tree;
  return stringMd5$1(JSON.stringify(mutateableDoc));
}

var uuid$1 = v4; // mimic old import, only v4 is ever used elsewhere
...
AbstractPouchDB.prototype.post =
  adapterFun$1('post', function (doc, opts, callback) {
  if (typeof opts === 'function') {
    callback = opts;
    opts = {};
  }
  if (typeof doc !== 'object' || Array.isArray(doc)) {
    return callback(createError$1(NOT_AN_OBJECT$1));
  }
...
AbstractPouchDB.prototype.put = adapterFun$1('put', function (doc, opts, cb) {
  ...
  if (typeof doc !== 'object' || Array.isArray(doc)) {
    return cb(createError$1(NOT_AN_OBJECT$1));
  }
...
AbstractPouchDB.prototype.putAttachment =
  adapterFun$1('putAttachment', function (docId, attachmentId, rev,
                                              blob, type) {
  var api = this;
  ...
  return api.get(docId).then(function (doc) {
    if (doc._rev !== rev) {
      throw createError$1(REV_CONFLICT$1);
    }
...
AbstractPouchDB.prototype.removeAttachment =
  ...
  self.get(docId, function (err, obj$$1) {
    ...
    if (obj$$1._rev !== rev) {
      callback(createError$1(REV_CONFLICT$1));
      return;
    }
    /* istanbul ignore if */
...
AbstractPouchDB.prototype.remove =
  adapterFun$1('remove', function (docOrId, optsOrRev, opts, callback) {
  var doc;
...
AbstractPouchDB.prototype.revsDiff =
  adapterFun$1('revsDiff', function (req, opts, callback) {
  if (typeof opts === 'function') {
    callback = opts;
    opts = {};
  }
...
AbstractPouchDB.prototype.bulkGet =
  adapterFun$1('bulkGet', function (opts, callback) {
...
AbstractPouchDB.prototype.compactDocument =
  adapterFun$1('compactDocument', function (docId, maxHeight, callback) {
  var self = this;
...
AbstractPouchDB.prototype.compact =
  adapterFun$1('compact', function (opts, callback) {
  if (typeof opts === 'function') {
    callback = opts;
    opts = {};
  }
...
AbstractPouchDB.prototype.get = adapterFun$1('get', function (id, opts, cb) {
  ...
  if (typeof id !== 'string') {
    return cb(createError$1(INVALID_ID$1));
  }
      ...
      if (Array.isArray(opts.open_revs)) {
        ...
        for (var i = 0; i < leaves.length; i++) {
          ...
          if (!(typeof (l) === "string" && /^\d+-/.test(l))) {
            return cb(createError$1(INVALID_REV$1));
          }
        }
        finishOpenRevs();
      } else {
        return cb(createError$1(UNKNOWN_ERROR$1, 'function_clause'));
      }
    }
...
AbstractPouchDB.prototype.getAttachment =
  adapterFun$1('getAttachment', function (docId, attachmentId, opts, callback) {
  var self = this;
  ...
  this._get(docId, opts, function (err, res) {
    ...
    if (res.doc._attachments && res.doc._attachments[attachmentId]) {
      ...
      return callback(createError$1(MISSING_DOC$1));
    }
  });
});

AbstractPouchDB.prototype.allDocs =
  adapterFun$1('allDocs', function (opts, callback) {
  if (typeof opts === 'function') {
    callback = opts;
    opts = {};
  }
  ...
  if ('keys' in opts) {
    ...
    if (incompatibleOpt) {
      callback(createError$1(QUERY_PARSE_ERROR$1,
...
AbstractPouchDB.prototype.bulkDocs =
  adapterFun$1('bulkDocs', function (req, opts, callback) {
  if (typeof opts === 'function') {
    callback = opts;
    opts = {};
  }
  ...
  if (!req || !req.docs || !Array.isArray(req.docs)) {
    return callback(createError$1(MISSING_BULK_DOCS$1));
  }
  ...
  if (attachmentError) {
    return callback(createError$1(BAD_REQUEST$1, attachmentError));
  }
...
AbstractPouchDB.prototype.registerDependentDatabase =
  adapterFun$1('registerDependentDatabase', function (dependentDb,
                                                          callback) {
  ...
  upsert$1(this, '_local/_pouch_dependentDbs', diffFun)
    .then(function () {
...
AbstractPouchDB.prototype.destroy =
  adapterFun$1('destroy', function (opts, callback) {
  ...
  if (!adapterName) { // automatically determine adapter
    for (var i = 0; i < preferredAdapters.length; ++i) {
      ...
      if (adapterName === 'idb' && 'websql' in adapters &&
          hasLocalStorage$1() && localStorage['_pouch__websqldb_' + prefix + name]) {
        // log it, because this can be confusing during development
        guardedConsole$1('log', 'PouchDB is downgrading "' + name + '" to WebSQL to' +
...
PouchDB.plugin = function (obj$$1) {
  ...
  if (this.__defaults) {
    PouchDB.__defaults = assign$2$$1({}, this.__defaults);
  }
  return PouchDB;
};
...
PouchDB.defaults = function (defaultOpts) {
  function PouchAlt(name, opts) {
    ...
    opts = assign$2$$1({}, PouchAlt.__defaults, opts);
  ...
  PouchAlt.__defaults = assign$2$$1({}, this.__defaults, defaultOpts);
...
function shouldFilter(changesHandler, opts) {
  return opts.filter && typeof opts.filter === 'string' &&
    !opts.doc_ids && !isRemote$1(changesHandler.db);
}
...
function filter(changesHandler, opts) {
  ...
  if (opts.filter === '_view') {
    if (!opts.view || typeof opts.view !== 'string') {
      var err = createError$1(BAD_REQUEST$1,
...
function allDocsKeysQuery(api, opts) {
  ...
  return Promise.all(keys.map(function (key) {
    var subOpts = assign$2$$1({key: key, deleted: 'ok'}, opts);
...
function parseRevisionInfo(rev) {
  if (!/^\d+-/.test(rev)) {
    return createError$1(INVALID_REV$1);
  }
  var idx = rev.indexOf('-');
  ...
  for (var key in doc) {
    ...
    if (Object.prototype.hasOwnProperty.call(doc, key)) {
      ...
      if (specialKey && !reservedWords[key]) {
        var error = createError$1(DOC_VALIDATION$1, key);
...
function updateDoc(revLimit, prev, docInfo, results,
  ...
  if (inConflict) {
    var err = createError$1(REV_CONFLICT$1);
    results[i] = err;
    return cb();
  }
...
function processDocs(revLimit, docInfos, api, fetchedDocs, tx, results,
  ...
  function insertDoc(docInfo, resultsIdx, callback) {
    ...
    if ('was_delete' in opts && deleted) {
      results[resultsIdx] = createError$1(MISSING_DOC$1, 'deleted');
      return callback();
    }
...
function LevelPouch(opts, callback) {
  ...
  api._get = readLock(function (id, opts, callback) {
    ...
    stores.docStore.get(id, function (err, metadata) {
      ...
      if (err || !metadata) {
        return callback(createError$1(MISSING_DOC$1, 'missing'));
      }
      ...
      if (!opts.rev) {
        ...
        if (deleted) {
          return callback(createError$1(MISSING_DOC$1, "deleted"));
        }
      } else {
  ...
  api._bulkDocs = writeLock(function (req, opts, callback) {
    ...
    function verifyAttachment(digest, callback) {
      txn.get(stores.attachmentStore, digest, function (levelErr) {
        if (levelErr) {
          var err = createError$1(MISSING_STUB$1,
    ...
    function writeDoc(docInfo, winningRev$$1, winningRevIsDeleted, newRevIsDeleted,
      ...
      function doMD5(doc, key, attachmentSaved) {
        return function (data) {
          binaryMd5$1(data, onMD5Load(doc, key, data, attachmentSaved));
        };
      }
      ...
      for (var i = 0; i < attachments.length; i++) {
        ...
        if (typeof att.data === 'string') {
          ...
          try {
            ...
            callback(createError$1(BAD_ARG$1,
  ...
  api._close = function (callback) {
    /* istanbul ignore if */
    if (db.isClosed()) {
      return callback(createError$1(NOT_OPEN$1));
    }
  ...
  api._getRevisionTree = function (docId, callback) {
    stores.docStore.get(docId, function (err, metadata) {
      if (err) {
        callback(createError$1(MISSING_DOC$1));
      } else {
  ...
  api._doCompactionNoLock = function (docId, revs, opts, callback) {
    ...
    txn.get(stores.docStore, docId, function (err, metadata) {
      ...
      function deleteOrphanedAttachments() {
        ...
        var refsToDelete = new ExportedMap$$1();
  ...
  api._putLocalNoLock = function (doc, opts, callback) {
    ...
    txn.get(stores.localStore, id, function (err, resp) {
      if (err && oldRev) {
        return callback(createError$1(REV_CONFLICT$1));
      }
...
function HttpPouch(opts, callback) {
  ...
  function adapterFun$$1(name, fun) {
    return adapterFun$1(name, getArguments(function (args) {
      setup().then(function () {
  ...
  api.bulkGet = adapterFun$1('bulkGet', function (opts, callback) {
    ...
    function doBulkGetShim() {
      ...
      for (var i = 0; i < numBatches; i++) {
        var subOpts = pick$1(opts, ['revs', 'attachments', 'binary', 'latest']);
  ...
  api.putAttachment = adapterFun$$1('putAttachment', function (docId, attachmentId,
    ...
    if (typeof blob === 'string') {
      ...
      try {
        ...
        return callback(createError$1(BAD_ARG$1,
                        'Attachment is not a valid base64 string'));
      }
  ...
  api._changes = function (opts) {
    ...
    var fetched = function (err, res) {
      ...
      if ((opts.continuous && !(limit && leftToFetch <= 0)) || !finished) {
        ...
        nextTick$1(function () { fetchData(lastFetchedSeq, fetched); });
      } else {
...
function replicate(src, target, opts, returnValue, result) {
  ...
  function completeReplication(fatalError) {
    ...
    if (fatalError) {
      ...
      fatalError = createError$1(fatalError);
...
function replicateWrapper(src, target, opts, callback) {
  ...
  if (opts.doc_ids && !Array.isArray(opts.doc_ids)) {
    throw createError$1(BAD_REQUEST$1,
...
function Sync(src, target, opts, callback) {
  ...
  var optsPush = opts.push ? assign$2$$1({}, opts, opts.push) : opts;
  var optsPull = opts.pull ? assign$2$$1({}, opts, opts.pull) : opts;
